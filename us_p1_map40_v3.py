#!/usr/bin/env python3
# us_p1_map40_v3.py -- US P1 benchmark selection mapping (READ-ONLY, 40 rows)
#
# v3 change vs v2: eligibility denominator + freshness columns.
#   Sky ruling 2026-07-22 -- the 60-bar requirement defines the eligible set,
#   so the invariant denominator is eligible_rows, not rows. Stocks with
#   stock_n < 60 (currently SPCX at 24) are reported as insufficient_bars
#   and excluded from the denominator; their card falls back to the approved
#   "accumulating N sessions" notice, never to a price-only chart.
#   Added columns: stock_last_bar, freshness_equal, eligible.
#   Selection logic, closes reading and schema handling are unchanged from v2.
#
...
#   both fail            -- FAIL_CLOSED
#
# Invariants (exit 2 if violated):
#   rows == 40
#   selected_benchmark    == eligible_rows
#   dates_equal           == eligible_rows
#   other_fail_closed     == 0   (fail-closed rows that are NOT insufficient_bars)
...
def main():
    ...
    # pass 1 -- read every stock's dates once, derive reference_date
    cache = {}
    lasts = []
    for rec in recs:
        t = str(rec["ticker"])
        cache[t] = read_dates(a.closes_dir, t)
        if cache[t][2] == "ok":
            lasts.append(cache[t][0][-1])
    ref_date = max(lasts) if lasts else None
    ...
    n_eligible = n_insuff = n_other_fail = 0
    fallback_detail = []
    insuff_detail = []
    stale_detail = []
    ...
    for rec in recs:
        ...
        s_dates, s_n, s_st = cache[ticker]
        if s_st != "ok":
            n_insuff += 1
            insuff_detail.append("%s:%s" % (ticker, s_st))
            sys.stdout.write("\t".join([ticker, str(prim), str(sec), "INSUFFICIENT_BARS",
                                        "stock_closes_%s" % s_st, str(s_n), "-",
                                        "false", "n/a", "0", "n/a"]) + "\n")
            continue
        ...
        if not eligible:
            ...
            sys.stdout.write("\t".join([ticker, str(prim), str(sec), "INSUFFICIENT_BARS",
                                        "bars_lt_60", str(s_n), s_last, "false",
                                        str(fresh).lower(), "0", "n/a"]) + "\n")
            continue
        ...
        sys.stdout.write("\t".join([ticker, str(prim), str(sec), selected, reason,
                                    str(s_n), s_last, "true", str(fresh).lower(),
                                    str(b_n), dates_equal]) + "\n")
